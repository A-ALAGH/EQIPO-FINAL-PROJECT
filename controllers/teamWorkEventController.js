...
module.exports.createTeamWorkEvent = async (req, res) => {
  ...
  try {
    const teamWorkEvent = await TeamWorkEvent.create({ date, heure, lieu, activité, participations, organisateur, nombre_places_disponibles });
...
module.exports.getTeamWorkEvent = async (req, res) => {
  try {
    const teamWorkEvents = await TeamWorkEvent.find();
